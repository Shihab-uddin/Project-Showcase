...
function grvthps_portfolio_shortcode( $atts ) {
    ob_start();
    ...
    if ( ! empty( $atts['category'] ) ) {
        $args['tax_query'] = [
            [
                'taxonomy' => 'grvthps_portfolio_category',
                'field'    => 'slug',
                'terms'    => $atts['category'],
            ],
        ];
    ...
    if ( $query->have_posts() ) {
        ...
        while ( $query->have_posts() ) {
            ...
            $url        = get_post_meta( $project_id, '_grvthps_project_url', true );
            $thumbnail  = get_the_post_thumbnail_url( $project_id, 'full' );
            ...
            <div class="sp-portfolio-item" style="background-image: url('<?php echo esc_url( $thumbnail ); ?>');">
                ...
                <div class="sp-portfolio-overlay">
                    <?php if ( $display_mode === 'preview' ) : ?>
                        <?php
                        // Create a signed URL for the preview
                        $nonce       = wp_create_nonce( 'grvthps_view_portfolio_' . $project_id );
                        $preview_url = add_query_arg(
                            [
                                'grvthps_project_id' => $project_id,
                                '_wpnonce'           => $nonce,
                            ],
                            home_url( '/portfolio-preview/' )
                        );
                        ?>
                        <a href="<?php echo esc_url( $preview_url ); ?>" target="_blank" class="sp-portfolio-button">Live Preview</a>
        ...
        echo '<p>' . esc_html__( 'No portfolio projects found.', 'project-showcase-by-gravth' ) . '</p>';
    }

    return ob_get_clean();
}
add_shortcode( 'grvthps_showcase', 'grvthps_portfolio_shortcode' );
